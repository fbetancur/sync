...
function init({ failSilently } = {}) {
    return _checkSupport()           // 1. Verifica soporte de IndexedDB
        ...
        .then(() => db.open({         // 3. Abre la base de datos
            server: 'enketo',
            version: 4,
            schema: { /* ... */ }
        }))
        .then((s) => { server = s; }) // 4. Guarda referencia al servidor
...
surveyStore = {
    // Obtener formulario
    get(enketoId) {
        return server.surveys.get(enketoId)
            .then(_firstItemOnly)
    ...
    update(survey) {
        return server.surveys.update(_serializeSurvey(survey))
    ...
    // Eliminar formulario
    remove(enketoId) {
        return surveyStore.get(enketoId)
...
function getScriptContent() {
    ...
    const partialScriptHash = crypto.createHash('md5')
        .update(partialScript)
        .digest('hex')
        .substring(0, 7);
    
    const configurationHash = crypto.createHash('md5')
            ...
            .then(cache => {
                ...
                return cache.addAll(
                    resources.map(resource => 
                        new Request(resource, { cache: 'reload' })
                    )
                );
            })
            ...
            .then(response => {
                ...
                .then(response => {
                    ...
                    caches.open(CACHES[0]).then(cache => {
                        console.log('Dynamically adding resource to cache', 
                                    event.request.url);
...
```javascript
function set(survey) {
    return connection.getFormParts(survey)  // Descargar del servidor
...
function _addBinaryDefaultsAndUpdateModel(survey) {
    ...
    const tasks = binaryDefaultElements.map(el => 
        connection.getMediaFile(el.getAttribute('src'))
            .then(result => {
    ...
    return connection.getFormPartsHash(survey)
        .then(version => {
            if (hash === version) {
                ...
                console.log('Cached survey is outdated!', 
                           'old:', hash, 'new:', version);
...
function _loadMedia(survey, targetContainers) {
    ...
    _getElementsGroupedBySrc(targetContainers).forEach(elements => {
        ...
        const request = store.survey.resource.get(survey.enketoId, src)
            .then(async resource => {
...
function _saveRecord(survey, draft, recordName, confirmed) {
    ...
    if (!recordName) {
        return _getRecordName()
            .then(name => _saveRecord(survey, draft, name, false));
    }
    ...
    return autoSavePromise
        ...
        .catch(error => {
            ...
            gui.alert(error.message || t('confirm.save.unkownerror'), 
                     'Save Error');
        });
}
```
...
const uploadQueue = async (
    ...
    const records = await Promise.all(
        displayableRecords.map(({ instanceId }) => 
    ...
    for await (const record of records) {
        try {
            ...
            errorMsg = error.message || 
                       gui.getErrorResponseMsg(error.status);
...
function backoff(callback) {
    ...
    const delay = Math.min(
        Math.pow(2, backoffAttempts) * 1000,
        MAX_BACKOFF_DELAY
    );
...
function _prepareFormDataArray(record) {
    ...
    const xmlData = xmlSerializer.serializeToString(
        recordDoc.documentElement
    );
...
function getFileUrl(subject) {
    return new Promise((resolve, reject) => {
        if (!subject) {
            ...
            else if (instanceAttachments && 
                     instanceAttachments[escapedSubject]) {
...
function getCurrentFiles() {
    const fileInputs = [
        ...document.querySelectorAll(
            'form.or input[type="file"], ' +
            'form.or input[type="text"][data-drawing="true"]'
        )
    ];
    const fileTasks = [];
    ...
    const _processNameAndSize = function(input, file) {
        if (file && file.name) {
            ...
            if (input.dataset.resized && input.dataset.resizedDataURI) {
                file = utils.dataUriToBlobSync(
                    input.dataset.resizedDataURI
                );
            }
    ...
    fileInputs.forEach(input => {
        if (input.type === 'file') {
            ...
            if (input.files[0]) {
                fileTasks.push(
                    Promise.resolve(
                        _processNameAndSize(input, input.files[0])
                    )
                );
            }
        } else if (input.value) {
            // Imágenes de canvas (dibujos)
            const canvas = input.closest('.question')
...
function _updateFile(table, id, file) {
    ...
    return Promise.reject(
        new Error('DataError. File not complete or ID not provided.')
    );
}
```
...
function replaceMediaSources(container, media, options = {}) {
    ...
    const elements = [
        ...container.querySelectorAll('[src], [href]')
    ];
...
function encryptRecord(form, record) {
    ...
    const publicKeyPem = 
        `-----BEGIN PUBLIC KEY-----${form.encryptionKey}-----END PUBLIC KEY-----`;
    const forgePublicKey = forge.pki.publicKeyFromPem(publicKeyPem);
    
    // 3. Encriptar clave simétrica con RSA
    const base64EncryptedSymmetricKey = 
        _rsaEncrypt(symmetricKey, forgePublicKey);
    ...
    return _encryptMediaFiles(record.files, symmetricKey, seed)
        ...
        .then(blobs => {
            ...
            const fileMd5s = blobs.map(blob =>
                `${blob.name.substring(0, blob.name.length - 4)}::${blob.md5}`
            );
            const elements = [
                form.id,
                form.version,
...
function Manifest(formId, formVersion) {
    ...
    this.getXmlStr = () => 
        new XMLSerializer().serializeToString(manifestEl);
    ...
    this.addMetaElement = (nodeName, content) => {
        ...
        const metaEl = metaPresent || 
            document.createElementNS(OPENROSA_XFORMS_NS, 'meta');
        const childEl = document.createElementNS(
            OPENROSA_XFORMS_NS, 
            nodeName
        );
...
La funcionalidad "last-saved" permite que un formulario acceda a los datos del último registro guardado, útil para:
...
function hasLastSavedInstance(survey) {
    return Array.isArray(survey.externalData) &&
                    ...
                    prevPromise.then(() =>
                        store.record.get(record.instanceId)
                            .then(record => {
                                const failedFiles = [];
                                const folderName = 
                                    `${name}_${_formatDate(record.created)}`;
                                ...
                                record.files.forEach(file => {
                                    tasks.push(
                                        utils.blobToArrayBuffer(file.item)
                                            .then(arrayBuffer => {
                                                folder.file(
                                                    file.name, 
                                                    arrayBuffer
...
function _showErrorOrAuthenticate(error) {
    ...
    if (error.status === 401) {
        ...
        window.location.href = 
            `${settings.loginUrl}?return_url=${encodeURIComponent(
                window.location.href
            )}`;
...
const dump = {
    resources() {
        server.resources.query().all().execute()
            .done(results => {
                ...
                results.forEach(item => {
                    if (item instanceof Blob) {
                        ...
                        console.log('resource string with length', 
                                   item.length);
                    }
                });
            });
    ...
    surveys() {
        server.surveys.query().all().execute()
            .done(results => {
    ...
    records() {
        server.records.query().all().execute()
            .done(results => {
    ...
    files() {
        server.files.query().all().execute()
            .done(results => {
                ...
                results.forEach(item => {
                    if (item instanceof Blob) {
                        ...
                        console.log('file string with length', 
                                   item.length);
                    }
                });
            });
...
3. Application → Cache Storage → enketo-common_*
...
*Este documento fue generado mediante análisis exhaustivo del código fuente de Enketo Express.*  
*Fecha: Diciembre 2024*  
*Versión analizada: Basada en estructura de archivos proporcionada*
