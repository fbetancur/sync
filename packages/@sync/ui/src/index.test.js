...
describe('@sync/ui hooks', () => {
  it('debería exportar hooks de utilidad', async () => {
    const { createEncryptionHook, useEncryption, createBackgroundSyncHook, useBackgroundSync } = await import('./hooks/useEncryption.js');
...
describe('@sync/ui stores', () => {
  it('debería exportar stores reactivos', async () => {
    const { syncStore, authStore, createSyncStore, createAuthStore } = await import('./stores/sync.js');
  ...
  it('debería crear store de sincronización con manager', async () => {
    ...
    const mockSyncManager = {
      sync: async () => ({ success: true, uploaded: 0, downloaded: 0, errors: [] }),
