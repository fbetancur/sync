...
function measureBuildPerformance() {
  ...
  for (const pkg of packages) {
    const result = execCommand(`pnpm --filter ${pkg} build`, `Building ${pkg}`);
  ...
  for (const app of apps) {
    const result = execCommand(`pnpm --filter ${app} build`, `Building ${app}`);
...
function measureInstallPerformance() {
  ...
  const cleanResult = execCommand('pnpm clean', 'Cleaning node_modules');

  if (!cleanResult.success) {
    log('yellow', '⚠️  No se pudo limpiar, midiendo reinstalación...');
  }

  const installResult = execCommand('pnpm install', 'Installing dependencies');
...
function printResults(results) {
  ...
  if (results.type === 'build') {
    if (results.total.duration > 120000) {
      // > 2 minutos
      log('yellow', '   ⚠️  Build time > 2 minutos - considera optimizaciones');
    } else if (results.total.duration < 30000) {
      // < 30 segundos
  ...
  if (results.type === 'test') {
    if (results.total.duration > 60000) {
      // > 1 minuto
      log('yellow', '   ⚠️  Test time > 1 minuto - considera paralelización');
    } else if (results.total.duration < 15000) {
      // < 15 segundos
  ...
  if (results.type === 'install') {
    if (results.total.duration > 180000) {
      // > 3 minutos
      log('yellow', '   ⚠️  Install time > 3 minutos - revisa dependencias');
    } else if (results.total.duration < 60000) {
      // < 1 minuto
...
function saveResults(results, outputDir) {
  ...
  const latestPath = path.join(
    outputDir,
    `performance-${results.type}-latest.json`
  );
...
function compareWithPrevious(results, outputDir) {
  ...
  try {
    ...
    const totalPercent = ((totalDiff / previous.total.duration) * 100).toFixed(
      1
    );
    ...
    log(
      color,
      `   Total: ${sign}${formatTime(totalDiff)} (${sign}${totalPercent}%)`
    );
...
function generateMarkdownReport(report) {
  ...
  const totalDuration = Object.values(report.summary).reduce(
