    ...
    try {
      switch (step.type) {
        ...
        case 'update_package_json':
          log('yellow', `   ⚠️  Revisar package.json manualmente si es necesario`);
          break;
          
        case 'run_tests':
          log('yellow', `   ⚠️  Ejecutar tests manualmente: pnpm test:packages`);
          break;
...
function printMigrationSummary(plan, success) {
  ...
  log('cyan', `📦 Destino: ${plan.packageName}${plan.moduleName ? '/' + plan.moduleName : ''}`);
...
function main() {
  ...
  if (!sourcePath || !packageName) {
    log('red', '❌ Uso: node tools/scripts/migrate-package.js <source-path> <package-name> [module-name]');
    log('yellow', '\\nEjemplos:');
    log('yellow', '  node tools/scripts/migrate-package.js src/lib/utils @sync/core utils');
    log('yellow', '  node tools/scripts/migrate-package.js apps/credisync/src/lib/components @sync/ui');
    log('yellow', '\\nOpciones:');
  ...
  const plan = generateMigrationPlan(sourcePath, packageName, moduleName, stats);
