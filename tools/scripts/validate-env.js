...
function validateApp(appName) {
  ...
  for (const varName of requiredVars) {
    if (!envLocal[varName] || envLocal[varName] === 'your_' + varName.toLowerCase().replace('vite_', '')) {
  ...
  for (const varName of Object.keys(envExample)) {
    if (!envLocal[varName]) {
      log('yellow', `⚠️  Variable en .env.example pero no en .env.local: ${varName}`);
    }
  }
...
function main() {
  ...
  if (targetApp) {
    ...
    if (!REQUIRED_VARS[targetApp]) {
      ...
      log('yellow', `Apps disponibles: ${Object.keys(REQUIRED_VARS).join(', ')}`);
      process.exit(1);
    }
