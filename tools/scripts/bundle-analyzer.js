...
function printAnalysis(analysis) {
  ...
  if (totalClientSize > 1024 * 1024) { // > 1MB
  ...
  if (totalClientSize < 500 * 1024) { // < 500KB
...
function saveAnalysis(analysis, outputDir) {
  ...
  const latestPath = path.join(outputDir, `bundle-analysis-${analysis.app}-latest.json`);
...
function compareWithPrevious(analysis, outputDir) {
  ...
  const previousPath = path.join(outputDir, `bundle-analysis-${analysis.app}-latest.json`);
  ...
  try {
    ...
    const clientColor = clientDiff > 0 ? 'red' : clientDiff < 0 ? 'green' : 'cyan';
...
function analyzeAllApps() {
  ...
  for (const app of apps) {
    ...
    if (analysis) {
      ...
      const outputDir = path.join(rootDir, 'tools', 'reports', 'bundle-analysis');
  ...
  if (results.length > 0) {
    ...
    log('cyan', `   Promedio por app: ${formatBytes(totalSize / results.length)}`);
  }
