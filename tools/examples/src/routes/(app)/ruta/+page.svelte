<script>
	...
	let clientes = $state([]);
	let clientesFiltrados = $state([]); // 👈 CAMBIO: No usar $derived
	...
	// 👇 CAMBIO: Filtrar solo cuando cambia la búsqueda
	$effect(() => {
		if (searchQuery === '') {
	...
	/**
	 * Cargar datos - Solo se ejecuta al montar o después de un pago
	 */
	async function cargarDatos() {
		try {
			...
			// ✅ OPTIMIZADO: Leer clientes con créditos activos (ya tienen estados)
			const todosLosClientes = await db.clientes
				.where('creditos_activos')
				.above(0)
				.toArray();
			
			// ✅ Para cada cliente, obtener sus créditos con cuotas del día
			const clientesConDatos = [];
			
			for (const cliente of todosLosClientes) {
				// ✅ Obtener créditos activos (ya tienen estados calculados)
				...
				// ✅ Enriquecer cada crédito con información de cuotas (ya calculadas)
				const creditosEnriquecidos = [];
				let totalAdeudado = 0;
				let cuotasAtrasadasTotal = 0;
				let diasAtrasoMax = 0;
				
				for (const credito of creditos) {
					// Obtener cuotas pendientes del crédito (ya tienen estados calculados)
					const cuotasPendientes = await db.cuotas
						.where('credito_id')
						.equals(credito.id)
						.and(c => c.estado !== 'PAGADA')
						.toArray();
					
					// Calcular adeudado del crédito (desde cuotas con estados)
					const adeudadoCredito = cuotasPendientes.reduce((sum, c) => sum + (c.saldo_pendiente || 0), 0);
					
					// Cuotas vencidas
					const cuotasVencidas = cuotasPendientes.filter(c => c.fecha_programada < hoy);
					const cuotasAtrasadas = cuotasVencidas.length;
					const diasAtraso = cuotasVencidas.length > 0 
						? Math.max(...cuotasVencidas.map(c => c.dias_atraso || 0))
						: 0;
					
					// Agregar al crédito
					creditosEnriquecidos.push({
						...credito,
						adeudado: adeudadoCredito,
						cuotas_atrasadas: cuotasAtrasadas,
						dias_atraso: diasAtraso,
						numero: credito.numero_credito || `Crédito ${credito.id.substring(0, 8)}`,
						tipo: credito.producto_nombre || 'Crédito'
					});
				...
				clientesConDatos.push({
					...
					resumen: {
						...
						cuotas_atrasadas: cuotasAtrasadasTotal,
						dias_atraso_max: diasAtrasoMax
					},
					estado: diasAtrasoMax > 0 ? 'MORA' : totalAdeudado > 0 ? 'PROXIMO' : 'AL_DIA',
					creditos: creditosEnriquecidos // ✅ Créditos con información completa
				});
			}
			
			clientes = clientesConDatos;
			clientesFiltrados = clientesConDatos;
	...
	async function calcularEstadisticas(hoy) {
		try {
			...
			// ✅ Créditos otorgados hoy (query optimizada)
			const creditosHoy = await db.creditos
				.where('fecha_desembolso')
				.equals(hoy)
				.toArray();
			const creditosOtorgadosHoy = creditosHoy.length;
			const montoOtorgadoHoy = creditosHoy.reduce((sum, c) => sum + (c.monto_original || 0), 0);
			
			// ✅ Montos vencidos y clientes en mora (ya calculado)
