...
export const tenantSchema = z
  ...
  .refine(data => data.usuarios_activos <= data.usuarios_contratados, {
...
export const productoCreditoSchema = z
  .object({
    id: uuidSchema,
    tenant_id: uuidSchema,
    nombre: z.string().min(1).max(100),
  ...
  .refine(data => data.plazo_maximo >= data.plazo_minimo, {
...
export const clienteFormSchema = clienteSchema
  .pick({
    nombre: true,
    tipo_documento: true,
...
export const pagoFormSchema = pagoSchema
  .pick({
    credito_id: true,
    cliente_id: true,
    monto: true,
