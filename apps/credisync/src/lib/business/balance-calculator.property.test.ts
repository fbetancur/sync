...
import { BalanceCalculator, type Pago, type Credito } from './balance-calculator';
      ...
      fc.assert(
        fc.property(
          ...
          fc.record({
            ...
            fecha_desembolso: fc.date({ min: new Date(2020, 0, 1), max: new Date(2025, 11, 31) }).map(d => d.getTime()),
          ...
          fc.array(
            fc.record({
              ...
              fecha: fc.date({ min: new Date(2020, 0, 1), max: new Date(2025, 11, 31) }).map(d => d.getTime()),
            }),
          ...
          (credito, pagos) => {
            ...
            const saldoCalculado = calculator.calculateSaldoPendiente(credito, creditoWithPagos);

            // Calculate expected saldo manually
            const totalPagado = creditoWithPagos.reduce((sum, pago) => sum + pago.monto, 0);
            const saldoEsperado = Math.max(0, credito.total_a_pagar - totalPagado);
      ...
      fc.assert(
        fc.property(
          ...
          (credito, pagos) => {
            ...
            const saldo = calculator.calculateSaldoPendiente(credito, creditoWithPagos);
      ...
      fc.assert(
        fc.property(
          ...
          (credito, pagos) => {
            ...
            const pagosForThisCredito = pagos.slice(0, Math.floor(pagos.length / 2)).map(p => ({
              ...p,
              credito_id: credito.id,
            }));
            const pagosForOtherCreditos = pagos.slice(Math.floor(pagos.length / 2));
            ...
            const totalPagadoThisCredito = pagosForThisCredito.reduce((sum, p) => sum + p.monto, 0);
            const expectedSaldo = Math.max(0, credito.total_a_pagar - totalPagadoThisCredito);
      ...
      fc.assert(
        fc.property(
          ...
          (credito, pagos) => {
            ...
            const saldo1 = calculator.calculateSaldoPendiente(credito, creditoWithPagos);

            // Calculate saldo with reversed order
            const saldo2 = calculator.calculateSaldoPendiente(credito, [...creditoWithPagos].reverse());
      ...
      fc.assert(
        fc.property(
          ...
          (credito, pagos) => {
            ...
            const cuotasPagadas = calculator.calculateCuotasPagadas(credito, creditoWithPagos);
      ...
      fc.assert(
        fc.property(
          ...
          (credito, pagos) => {
            ...
            const cuotasPagadas2 = calculator.calculateCuotasPagadas(credito, creditoWithPagos);
