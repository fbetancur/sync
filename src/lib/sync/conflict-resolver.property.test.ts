...
const versionVectorArb = fc.dictionary(deviceIdArb, fc.nat(100), { minKeys: 1, maxKeys: 5 });
		...
		it('should produce same result regardless of resolution order', () => {
			fc.assert(
				fc.property(clienteRecordArb, clienteRecordArb, (local, remote) => {
					...
					const fields1 = Object.keys(result1.resolved.field_versions || {}).sort();
					const fields2 = Object.keys(result2.resolved.field_versions || {}).sort();
		...
		it('should handle version vector dominance correctly', () => {
			fc.assert(
				fc.property(versionVectorArb, versionVectorArb, (vv1, vv2) => {
					...
					expect(['local_wins', 'remote_wins', 'merged']).toContain(result.strategy);
				}),
				{ numRuns: 100 }
			);
		});
		...
		it('should maintain CRDT properties (commutativity)', () => {
			fc.assert(
				fc.property(clienteRecordArb, clienteRecordArb, (record1, record2) => {
					...
					// Resolve in both orders
					const resultAB = resolver.resolveConflict(record1, record2, 'clientes');
					const resultBA = resolver.resolveConflict(record2, record1, 'clientes');
			...
			fc.assert(
				fc.property(
					...
					(record, fieldName, newValue, timestamp) => {
						...
						expect(result.resolved.field_versions[fieldName].value).toBe('remote-value');
						expect(result.resolved.field_versions[fieldName].device_id).toBe('device-2');
